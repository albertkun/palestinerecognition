...
class ChartControl {
	...
	updateData(data) {
		...
		title.textContent = "Countries that recognize Palestine in " + currentYear; // Set the title text
		title.style.fontFamily = "'Roboto', sans-serif"; // Use the Roboto Google Font
		...
		source.style.fontFamily = "'Roboto', sans-serif"; // Use the Roboto Google Font
...
map.on('load', function () {
    ...
    map.on('mousemove', 'countries-layer', function(e) {
		...
		// Display a popup with the name of the country.
		if (feature.properties.recognition === 'yes') {
			html += '<br>Year recognized: ' + new Date(feature.properties.date).getFullYear();
		}
